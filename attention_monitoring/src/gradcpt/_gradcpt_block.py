...
import polars as pl
import pyxdf
from attention_monitoring.src.study._study_session import StudySession
...
class GradCPTBlock(StudyBlock):
    def __init__(
        ...
        if _name is None:
            ...
            self._stimSequenceFile = os.path.join(
                self._DIR, self.name + "_stim_sequence.csv"
                )
            self._dataFile = os.path.join(
                self._DIR, self.name + "_data.xdf"
                )
            self._info.update(
                stim_sequence_file=self._stimSequenceFile,
                data_file=self._dataFile
                )
            
            # Create the stim sequence file
            self.__generateStimSequence
        else:
            # Loading an existing block
            pass

    @classmethod
    def makePracticeBlock(cls, *args, **kwargs) -> Self:
        pass
    
    @classmethod
    def makeFullBlock(cls, *args, **kwargs) -> Self:
        pass
    ...
    def run(self) -> None:
        raise NotImplementedError(
            "gradCPT blocks cannot be run directly, but can be ran indirectly "
            + "by running the parent gradCPT session."
            )
    ...
    def __generateStimSequence(
        ...
        with open(name + ".csv", 'x', newline="") as f:
